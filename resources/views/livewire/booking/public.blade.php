...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-6 sm:p-8 mb-6 text-center relative overflow-hidden">
        <div class="absolute -top-12 -right-12 w-36 h-36 bg-gradient-to-br from-mulberry-100 to-mulberry-200 rounded-full blur-2xl opacity-70 pointer-events-none"></div>
        <div class="absolute -bottom-12 -left-12 w-36 h-36 bg-gradient-to-tr from-amber-100 to-mulberry-100 rounded-full blur-2xl opacity-70 pointer-events-none"></div>

        <div class="relative z-10">
            <div class="w-16 h-16 sm:w-20 sm:h-20 mx-auto rounded-2xl bg-gradient-to-br from-mulberry-500 to-mulberry-600 text-white flex items-center justify-center text-2xl sm:text-3xl font-bold shadow-md shadow-mulberry-500/20 mb-3">
                {{ substr($biz->name, 0, 1) }}
            </div>
            ...
            @if($biz->niche)
                <p class="text-sm font-medium text-gray-500 mt-1 capitalize">{{ $biz->niche }} &middot; Online Booking</p>
            @endif

            <div class="flex flex-wrap items-center justify-center gap-4 text-xs sm:text-sm text-gray-600 mt-3 pt-3 border-t border-gray-100">
                @if($biz->address)
                ...
                @endif
                @if($biz->phone)
                    <a href="tel:{{ $biz->phone }}" class="flex items-center gap-1.5 text-mulberry-600 hover:text-mulberry-700 font-medium">
    ...
    @if($step < 5)
        <!-- Progress Steps Bar -->
        <nav class="mb-6 bg-white rounded-xl p-3 border border-gray-200 shadow-sm">
            <ol class="flex items-center justify-between text-xs font-semibold text-gray-500">
                <li class="flex items-center gap-1.5 cursor-pointer {{ $step >= 1 ? 'text-mulberry-600' : '' }}" wire:click="goToStep(1)">
                    <span class="w-5 h-5 rounded-full flex items-center justify-center text-[11px] {{ $step >= 1 ? 'bg-mulberry-600 text-white' : 'bg-gray-100 text-gray-500' }}">1</span>
                ...
                <li class="flex items-center gap-1.5 cursor-pointer {{ $step >= 2 ? 'text-mulberry-600' : '' }}" wire:click="goToStep(2)">
                    <span class="w-5 h-5 rounded-full flex items-center justify-center text-[11px] {{ $step >= 2 ? 'bg-mulberry-600 text-white' : 'bg-gray-100 text-gray-500' }}">2</span>
                    <span class="hidden sm:inline">Staff</span>
                </li>
                <svg class="w-3.5 h-3.5 text-gray-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"/></svg>

                <li class="flex items-center gap-1.5 cursor-pointer {{ $step >= 3 ? 'text-mulberry-600' : '' }}" wire:click="goToStep(3)">
                    <span class="w-5 h-5 rounded-full flex items-center justify-center text-[11px] {{ $step >= 3 ? 'bg-mulberry-600 text-white' : 'bg-gray-100 text-gray-500' }}">3</span>
                    <span class="hidden sm:inline">Date & Time</span>
                </li>
                <svg class="w-3.5 h-3.5 text-gray-300" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"/></svg>

                <li class="flex items-center gap-1.5 {{ $step >= 4 ? 'text-mulberry-600' : '' }}">
                    <span class="w-5 h-5 rounded-full flex items-center justify-center text-[11px] {{ $step >= 4 ? 'bg-mulberry-600 text-white' : 'bg-gray-100 text-gray-500' }}">4</span>
    ...
    @if($step === 1)
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5 sm:p-7">
            <h2 class="text-xl font-bold text-gray-900 mb-1">Select a Service</h2>
            <p class="text-sm text-gray-500 mb-5">Choose the service you'd like to book today.</p>

            <div class="space-y-3">
                @forelse($services as $svc)
                    <div wire:click="selectService({{ $svc->id }})"
                         class="group p-4 sm:p-5 rounded-xl border border-gray-200 hover:border-mulberry-400 hover:bg-mulberry-50/30 transition-all cursor-pointer flex items-center justify-between gap-4 {{ $selectedServiceId === $svc->id ? 'ring-2 ring-mulberry-500 border-mulberry-500 bg-mulberry-50/40' : '' }}">
                        <div class="min-w-0">
                            <h3 class="text-base font-bold text-gray-900 group-hover:text-mulberry-600 transition-colors">
                                {{ $svc->name }}
                            </h3>
                            @if($svc->description)
                                <p class="text-xs sm:text-sm text-gray-500 mt-0.5 line-clamp-2">{{ $svc->description }}</p>
                            @endif
                            <div class="flex items-center gap-3 mt-2 text-xs font-semibold text-gray-600">
                        ...
                        <div class="text-right shrink-0">
                            <span class="text-lg sm:text-xl font-extrabold text-gray-900 group-hover:text-mulberry-600">
                                &pound;{{ number_format($svc->price, 2) }}
                            </span>
                            <div class="mt-1">
                                <span class="inline-flex items-center text-xs font-semibold text-mulberry-600 group-hover:underline">
                                    Book &rarr;
                                </span>
                            </div>
                        </div>
    ...
    @if($step === 2)
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5 sm:p-7">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Select Specialist</h2>
                    <p class="text-sm text-gray-500">Choose who you'd like to perform your {{ $selectedService?->name }}.</p>
                </div>
                <button type="button" wire:click="goToStep(1)" class="text-xs font-semibold text-gray-500 hover:text-gray-800">
                    &larr; Back
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                ...
                <div wire:click="selectStaff(null)"
                     class="p-4 rounded-xl border border-gray-200 hover:border-mulberry-400 hover:bg-mulberry-50/30 transition-all cursor-pointer flex items-center gap-3.5 {{ $selectedStaffId === null ? 'ring-2 ring-mulberry-500 border-mulberry-500 bg-mulberry-50/40' : '' }}">
                    <div class="w-11 h-11 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center font-bold text-sm shrink-0">
                        <svg class="w-5 h-5 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                        </svg>
                    </div>
                    <div>
                        <h4 class="text-sm font-bold text-gray-900">Any Available Specialist</h4>
                        <p class="text-xs text-gray-500">Fastest availability</p>
                    </div>
                </div>
                ...
                @foreach($staffMembers as $staff)
                    <div wire:click="selectStaff({{ $staff->id }})"
                         class="p-4 rounded-xl border border-gray-200 hover:border-mulberry-400 hover:bg-mulberry-50/30 transition-all cursor-pointer flex items-center gap-3.5 {{ $selectedStaffId === $staff->id ? 'ring-2 ring-mulberry-500 border-mulberry-500 bg-mulberry-50/40' : '' }}">
                        ...
                        <div>
                            <h4 class="text-sm font-bold text-gray-900">{{ $staff->name }}</h4>
    ...
    @if($step === 3)
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5 sm:p-7">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Pick Date & Time</h2>
                    <p class="text-sm text-gray-500">Service: <span class="font-semibold text-gray-800">{{ $selectedService?->name }}</span> ({{ $selectedService?->duration_minutes }}m) &middot; Specialist: <span class="font-semibold text-gray-800">{{ $selectedStaff ? $selectedStaff->name : 'Any Available' }}</span></p>
                </div>
                <button type="button" wire:click="goToStep(2)" class="text-xs font-semibold text-gray-500 hover:text-gray-800">
                    &larr; Back
                </button>
            </div>
            ...
            <div class="mt-4 mb-6">
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Select Day</label>
                <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
                    ...
                    @for($i = 0; $i < 14; $i++)
                        ...
                        <button type="button"
                                ...
                                class="flex-shrink-0 w-16 sm:w-20 py-3 px-2 rounded-xl text-center border transition-all {{ $isChosen ? 'border-mulberry-500 bg-mulberry-600 text-white shadow-md shadow-mulberry-500/20' : 'border-gray-200 bg-white hover:border-gray-300 text-gray-700' }}">
                            <div class="text-[11px] font-semibold uppercase {{ $isChosen ? 'text-mulberry-100' : 'text-gray-400' }}">
                                {{ $i === 0 ? 'Today' : ($i === 1 ? 'Tmrw' : $candidate->format('D')) }}
                            </div>
                            <div class="text-lg font-extrabold mt-0.5">
                                {{ $candidate->format('j') }}
                            </div>
                            <div class="text-[10px] {{ $isChosen ? 'text-mulberry-100' : 'text-gray-400' }}">
                                {{ $candidate->format('M') }}
                            </div>
                        </button>
                    @endfor
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 sm:gap-2.5">
                        @foreach($slots as $slot)
                            <button type="button"
                                    ...
                                    class="py-2.5 px-3 rounded-lg text-sm font-semibold border transition-all text-center {{ $selectedTime === $slot['time'] ? 'border-mulberry-500 bg-mulberry-500 text-white shadow-sm' : 'border-gray-200 hover:border-mulberry-300 hover:bg-mulberry-50/50 text-gray-800 bg-gray-50/50' }}">
                                {{ $slot['formatted'] }}
                            </button>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200 text-gray-500 text-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-5 sm:p-7">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Provide your contact info to confirm the appointment.</p>
                </div>
                <button type="button" wire:click="goToStep(3)" class="text-xs font-semibold text-gray-500 hover:text-gray-800">
                    &larr; Back
                </button>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 mb-6">
                <div class="flex flex-wrap items-center justify-between gap-4 pb-3 border-b border-gray-200">
                    <div>
                        <div class="text-xs text-gray-500 font-medium">Selected Booking</div>
                        <div class="text-base font-bold text-gray-900 mt-0.5">{{ $selectedService?->name }}</div>
                    ...
                    <div class="text-right">
                        <div class="text-xs text-gray-500 font-medium">Total Service Price</div>
                        <div class="text-xl font-extrabold text-gray-900">&pound;{{ number_format($selectedService?->price, 2) }}</div>
                    </div>
                </div>
                ...
                @if($depositActive && $depositDue > 0)
                    <div class="pt-3 flex items-center justify-between text-xs sm:text-sm">
                        <div>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-md font-bold bg-mulberry-100 text-mulberry-800 text-[11px]">
                        ...
                        <div class="text-right">
                            <span class="text-xs text-gray-500 font-medium block">Due Right Now</span>
                            <span class="text-lg font-black text-mulberry-600">&pound;{{ number_format($depositDue, 2) }}</span>
                        </div>
                    </div>
                @endif
            </div>
    ...
    @if($step === 5)
        <div class="bg-white rounded-3xl shadow-lg border border-gray-200/80 p-6 sm:p-10 text-center relative overflow-hidden">
            ...
            <p class="text-sm text-gray-600 mt-2 max-w-md mx-auto">
                We've reserved your slot at <span class="font-semibold text-gray-900">{{ $biz->name }}</span>.
            </p>

            <!-- Appointment Recap Card -->
            <div class="bg-gray-50 rounded-2xl p-5 border border-gray-200/80 text-left my-6 max-w-md mx-auto">
                <div class="flex items-center justify-between pb-3 border-b border-gray-200">
                    <div>
                        <div class="text-xs text-gray-500">Service</div>
                        <div class="font-bold text-gray-900 text-base">{{ $selectedService?->name }}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-xs text-gray-500">Price</div>
                        <div class="font-extrabold text-gray-900 text-base">&pound;{{ number_format($selectedService?->price, 2) }}</div>
                    </div>
                </div>

                <div class="pt-3 space-y-2 text-xs sm:text-sm text-gray-700">
                    ...
                    @if($confirmedApt && $confirmedApt->deposit_required)
                        <div class="mt-3 pt-3 border-t border-gray-200 flex items-center justify-between text-xs">
                            ...
                            <div class="text-right">
                                ...
                                <span class="font-bold text-gray-900">&pound;{{ number_format($confirmedApt->balanceDue(), 2) }}</span>
                            </div>
                        </div>
                    @endif
                </div>
            ...
            <div class="p-4 rounded-2xl bg-gradient-to-r from-gray-900 to-gray-800 text-white text-left max-w-md mx-auto mb-4 shadow-md">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-300">Sync with Google, Apple, or Outlook with 1 tap</p>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row gap-2 pt-1">
                    @if($googleCalendarUrl)
                        <a href="{{ $googleCalendarUrl }}" target="_blank"
                           class="flex-1 py-2.5 px-3 rounded-lg bg-mulberry-600 hover:bg-mulberry-500 text-white text-xs font-bold text-center transition-colors shadow-sm flex items-center justify-center gap-1.5">
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 text-xs text-gray-400">
                Need to make changes? Call <span class="font-semibold text-gray-600">{{ $biz->phone ?: $biz->name }}</span>.
                @if($cancellationUrl)
